...
func index(res http.ResponseWriter, req *http.Request) {
	...
	log.Println("INDEX 1: ", session.Values["logged_in"])
...
func login(res http.ResponseWriter, req *http.Request) {
	...
	if req.Method == "POST" {
		...
		log.Println("LOGIN 1: ", session.Values["logged_in"])
		// redirect to main page
		http.Redirect(res, req, "/", 302)
		}

	log.Println("LOGIN 2: ", session.Values["logged_in"])
	// Execute template
...
func logout(res http.ResponseWriter, req *http.Request) {
	...
	session.Save(req, res)
	log.Println("LOGOUT 1: ", session.Values["logged_in"])
	http.Redirect(res, req, "/", 302)
}
...
func upload(res http.ResponseWriter, req *http.Request) {
	...
	if session.Values["logged_in"] == false ||
		...
		log.Println("UPLOAD 1: ", session.Values["logged_in"])
	...
	if req.Method == "POST" {
		...
		log.Println("UPLOAD 2: ", session.Values["logged_in"])
		http.Redirect(res, req, "/", 302)
	}

	log.Println("UPLOAD 3: ", session.Values["logged_in"])
	// Execute template
